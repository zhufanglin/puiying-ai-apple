...
export default function CreateAwardPage() {
  ...
  const handleCreateAndReturn = async () => {
    ...
    try {
      ...
      setFieldErrors({ recipients: e.message || "建立失敗，請檢查填寫內容" });
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <FormSection title="基本資訊" subtitle="選擇範本和填寫獎狀資訊">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              {templates.length === 0 && (
                <p className="mt-1 text-xs text-orange-600 bg-orange-50 border border-orange-200 rounded px-3 py-2">
                  暫無可用範本，請先在範本管理中啟用至少一個獎狀範本
                </p>
              )}
