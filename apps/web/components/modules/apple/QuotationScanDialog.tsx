...
export default function QuotationScanDialog({ open, onClose, onSuccess }: Props) {
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      setProjectName(parsed.fields.purpose || "");
      setVendor(parsed.fields.payer || "");
      setStep("review");
    } catch (err) {
  ...
  const confirm = async () => {
    ...
    try {
      await api.post("/apple/finance/quotations", {
        project_name: projectName.trim() || edit?.purpose || "未指定項目",
        vendor: vendor.trim() || edit?.payer || "未知供應商",
      ...
      <div className="w-full max-w-[480px]" onClick={(e) => e.stopPropagation()}>
        <div className="bg-white rounded-lg border border-[#d8dee6]" style={{ boxShadow: "0 10px 30px rgba(16,24,40,0.08)" }}>
          <div className="flex items-center justify-between px-4 py-3 border-b border-[#d8dee6]">
            <h3 className="text-[16px] font-bold text-[#1d2939]">拍照識別報價單</h3>
            <button onClick={close} className="p-1 rounded hover:bg-[#f1f5f8] text-[#667085]"><X size={20} /></button>
          </div>
          <div className="p-4">
            {step === "upload" && (
              <div className="space-y-3">
                <p className="text-sm text-[#667085]">拍照或上傳報價單，自動識別項目名、報價單位、金額</p>
                <UploadDropzone accept="image/*,.pdf" maxSizeMB={10} onUpload={handleUpload} uploading={uploading} label="拖拽報價單到此處，或點擊上傳" />
                {uploading && (
            ...
            {step === "review" && ocr && edit && (
              <div className="space-y-3">
                ...
                <div className="space-y-3">
                  <div><label className={labelClass}>項目名稱</label><input type="text" value={projectName} onChange={e => setProjectName(e.target.value)} placeholder="如：校慶紀念品採購" className={inputClass} /></div>
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className={labelClass}>報價單位</label><input type="text" value={vendor} onChange={e => setVendor(e.target.value)} placeholder="如：精美禮品公司" className={inputClass} /></div>
                    <div><label className={labelClass}>報價金額 {edit.amount === null && <span className="text-[#b42318] text-xs">（待填寫）</span>}</label><input type="number" value={edit.amount ?? ""} onChange={e => setEdit({...edit, amount: e.target.value ? parseFloat(e.target.value) : null})} placeholder="0.00" className={inputClass} /></div>
                  </div>
                ...
                <div className="flex justify-end gap-2 pt-2 border-t border-[#d8dee6]">
                  ...
                  <button onClick={confirm} disabled={submitting} style={{ background: B }} className="px-4 py-[8px] text-sm text-white rounded-lg font-bold hover:opacity-90 disabled:opacity-50">{submitting ? "提交中..." : "確認"}</button>
                </div>
              </div>
            )}
          </div>
