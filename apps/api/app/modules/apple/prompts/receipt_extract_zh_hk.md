...
`ocr_confidence` 為 0 到 1；`lines[].confidence` 為 0 到 100。`lines` 保留原始閱讀順序及坐標，僅用於判斷標籤和值的關係，`raw_text` 仍必須逐字等於 `ocr_text`。
...
11. 標籤和值可以分處相鄰行。例如「日期：」下一行是日期值；「今收到／收到」本身只是標籤或句式，不可直接作為 `payer`，其下一個清晰姓名行才是付款人候選。
12. `N°`、`No.`、`Receipt No.`、`收據編號`、`收据编号` 後的純數字只能視為編號，絕不可視為金額；即使它是全文唯一的數字也不例外。
...
```json
{
  ...
  "lines": [
    ...
    {"line_no": 2, "text": "N°", "confidence": 91.0},
    {"line_no": 3, "text": "8865431", "confidence": 94.0},
    {"line_no": 4, "text": "日期：", "confidence": 96.0},
    {"line_no": 5, "text": "2025年6月15日", "confidence": 97.0},
    {"line_no": 6, "text": "今收到", "confidence": 92.0},
    {"line_no": 7, "text": "張三", "confidence": 90.0}
  ]
}
```
