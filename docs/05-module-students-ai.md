...
收据 Prompt 只要求模型返回 `fields`、`confidence` 和 `warnings`；`raw_text` 由服务端直接从受控 OCR 请求补回。模型结果先经过字段白名单归一化，再进入严格 Schema 和原文证据门。无法解析的格式只重试一次，且不会重发第一次模型输出；日志只记录固定原因码和字段路径，不记录 API Key、OCR 原文或模型正文。
