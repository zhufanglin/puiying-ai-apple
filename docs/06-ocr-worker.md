...
测试使用模拟百度和 DeepSeek 响应，不需要真实 AK/SK 或模型 Key，覆盖鉴权、token 缓存、错误处理、白名单归一化、一次格式重试、歧义日期、异常金额、标签错位、收据编号排除、模型幻觉字段清除、Key 脱敏和前端结果契约。
